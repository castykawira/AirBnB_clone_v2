...
class TestBaseModel(unittest.TestCase):
    ...
    def test_printing_instance_equals_string_format(self):
        """Check if the printing the instance of BaseModel equals “[<class name>] (<self.id>) <self.__dict__>” string format"""
        base_model = BaseModel()
    ...
    def test_inequality_of_created_at_and_updated_at_after_save(self):
        """Check the inequality of created_at and updated_at attributes after using the save(self) method"""
        base_model = BaseModel()
        old_created_at = base_model.created_at
        old_updated_at = base_model.updated_at
        base_model.save()
        self.assertNotEqual(old_created_at, base_model.created_at)
    ...
    def test_inequality_of_updated_at_before_and_after_save(self):
        """Check the inequality of updated_at and updated_at attributes before and after using the save(self) method"""
        base_model = BaseModel()
        old_updated_at = base_model.updated_at
        base_model.save()
        self.assertNotEqual(old_updated_at, base_model.updated_at)
    ...
    def test_datetime_format_in_to_dict(self):
        """Check if created_at and updated_at are in the “%Y-%m-%dT%H:%M:%S.%f” ISO format in the dictionary returned from to_dict method"""
        base_model = BaseModel()
        obj_dict = base_model.to_dict()
        self.assertEqual(datetime.strptime(obj_dict['created_at'], "%Y-%m-%dT%H:%M:%S.%f"), base_model.created_at)
        self.assertEqual(datetime.strptime(obj_dict['updated_at'], "%Y-%m-%dT%H:%M:%S.%f"), base_model.updated_at)

    def test_updated_at_after_save_is_greater_than_old_updated_at(self):
        """Check that the updated_at after using the save method is greater than the old updated_at"""
        base_model = BaseModel()
        old_updated_at = base_model.updated_at
        base_model.save()
    ...
    def test_to_dict_method_returns_correct_keys_and_values(self):
        """Check if to_dict method returned a dictionary that has the correct keys and values"""
        base_model = BaseModel()
        obj_dict = base_model.to_dict()
    ...
    def test_to_dict_method_returns_dictionary(self):
        """Test the to_dict method, e.g., check if instance of returned obj is a dictionary, check if the returned obj has a key as a classname (__class__) like assertIn(‘__class__’, obj_dict)"""
        base_model = BaseModel()
        obj_dict = base_model.to_dict()
